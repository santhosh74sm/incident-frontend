...
const normalizeOptions = (options = []) =>
    options.map((option) =>
        typeof option === 'string'
            ...
            : { id: option?.id ?? option?.value ?? option?.name, label: option?.label ?? option?.name ?? String(option?.id ?? '') }
    );
...
const buildDropdownLayout = (triggerElement) => {
    ...
    const availableHeight = Math.max(
        220,
        Math.min(openAbove ? spaceAbove - 8 : spaceBelow - 8, 420)
    );
    const left = Math.min(
...
export const FilterDropdown = ({
    ...
    const dropdownContent = isOpen && portalReady && dropdownLayout
        ? createPortal(
            <div
                ref={menuRef}
                id={listboxId}
                ...
                <div className="border-b border-slate-200 bg-white px-4 py-2.5 dark:border-slate-800 dark:bg-slate-900">
                    <label className="flex min-h-[44px] cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800">
                        <input
                            ...
                            onChange={(event) => onChange(event.target.checked ? normalizedOptions.map((option) => option.id) : [])}
        ...
        <div className="relative min-w-0">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    {hasActiveSelection ? (
                        <span className="inline-flex min-w-[1.75rem] items-center justify-center rounded-full bg-blue-100 px-2 py-0.5 text-[11px] font-semibold text-blue-700 dark:bg-blue-500/20 dark:text-blue-100">
                            {selected.length}
                        </span>
                    ) : null}
    ...
    <div className="overflow-visible rounded-[28px] border border-white/70 bg-white/90 shadow-md shadow-slate-200/70 backdrop-blur transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900/90 dark:shadow-slate-950/50">
        <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-3">
                <div className="rounded-2xl border border-blue-100 bg-blue-50 p-2.5 text-blue-600 shadow-sm dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-200">
                    <Filter className="h-4 w-4" />
                </div>
                <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-700 dark:text-slate-200">{title}</h3>
                    {hasActiveFilters ? (
