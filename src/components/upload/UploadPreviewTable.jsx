...
const UploadPreviewTable = ({ preview }) => {
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="flex flex-col gap-3 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-blue-50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div
                    ...
                    className="inline-flex shrink-0 items-center gap-2 rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 "
                >
                ...
                <table className="min-w-[640px] w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100 ">
                        {preview.rows?.map((row) => (
                            ...
                            >
                                <td className="py-2.5 pl-3 pr-4 font-semibold text-slate-400 ">
                                    {row.__rowNumber}
                                </td>
                                {columns.map((col) => (
                    ...
                    >
                        ...
                        <div className="divide-y divide-slate-100 ">
                            {columns.map((col) => (
                                ...
                                >
                                    <span className="min-w-0 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500 ">
                                        {col}
                                    </span>
                        ...
                        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5 ">
                            ...
                            <ul className="mt-2.5 space-y-1.5 pl-1">
                                {preview.rowIssues?.map((issue) => (
                                    <li key={`issue-${issue.row}`} className="text-sm text-amber-900 ">
